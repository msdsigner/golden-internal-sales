...
try:
    ...
    print("[Warning] 'scripts.excel_utils' missing. Falling back to basic categorization.")
    def load_raw_data(f): return []
    def classify_category(pc, ir, n, **kwargs): return "Uncategorized"
...
def run(interactive=True):
    ...
    # Update this section in your run() function:
    raw_data.sort(
        key=lambda x: (
            classify_category(
                ...
                str(x.get("Name", "")),
                condition=str(x.get("Condition", "")) # Added condition here
            ),
            str(x.get("Internal Reference", ""))
        )
    )
